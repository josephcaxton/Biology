...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (tableView.tag == 1) {
        ...
        if (TVHeaderImageView == nil) {
            ...
            NSString *HeaderImagePath = [[NSBundle mainBundle] pathForResource:@"EQ_biology_header" ofType:@"png"];
...
- (void)reviewPressed {
    ...
    // Report to  analytics
    NSError *error;
    if (![[GANTracker sharedTracker] trackEvent:@"User Sent to Review biology Question iPad at app store"
                                         action:@"User Sent to Review biology Question iPad at app store"
                                          label:@"User Sent to Review biology Question iPad at app store"
                                          value:1
                                      withError:&error]) {
